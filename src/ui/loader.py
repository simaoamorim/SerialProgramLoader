# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'loader.ui'
#
# Created by: PyQt5 UI code generator 5.15.1
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets


class Ui_Loader(object):
    def setupUi(self, Loader):
        Loader.setObjectName("Loader")
        Loader.resize(400, 300)
        font = QtGui.QFont()
        font.setPointSize(8)
        Loader.setFont(font)
        self.gridLayout_2 = QtWidgets.QGridLayout(Loader)
        self.gridLayout_2.setObjectName("gridLayout_2")
        self.label = QtWidgets.QLabel(Loader)
        self.label.setObjectName("label")
        self.gridLayout_2.addWidget(self.label, 0, 0, 1, 1)
        self.verticalLayout = QtWidgets.QVBoxLayout()
        self.verticalLayout.setSpacing(7)
        self.verticalLayout.setObjectName("verticalLayout")
        self.serialPortChooser = QtWidgets.QComboBox(Loader)
        self.serialPortChooser.setObjectName("serialPortChooser")
        self.verticalLayout.addWidget(self.serialPortChooser)
        spacerItem = QtWidgets.QSpacerItem(20, 40, QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Expanding)
        self.verticalLayout.addItem(spacerItem)
        self.sendButton = QtWidgets.QPushButton(Loader)
        self.sendButton.setEnabled(False)
        self.sendButton.setObjectName("sendButton")
        self.verticalLayout.addWidget(self.sendButton)
        self.gridLayout_2.addLayout(self.verticalLayout, 1, 2, 1, 1)
        self.programListWidget = QtWidgets.QListWidget(Loader)
        self.programListWidget.setSelectionRectVisible(True)
        self.programListWidget.setObjectName("programListWidget")
        self.gridLayout_2.addWidget(self.programListWidget, 1, 0, 1, 1)
        self.updateProgramListButton = QtWidgets.QPushButton(Loader)
        self.updateProgramListButton.setObjectName("updateProgramListButton")
        self.gridLayout_2.addWidget(self.updateProgramListButton, 0, 2, 1, 1)

        self.retranslateUi(Loader)
        QtCore.QMetaObject.connectSlotsByName(Loader)

    def retranslateUi(self, Loader):
        _translate = QtCore.QCoreApplication.translate
        Loader.setWindowTitle(_translate("Loader", "Form"))
        self.label.setText(_translate("Loader", "TextLabel"))
        self.sendButton.setText(_translate("Loader", "Send"))
        self.updateProgramListButton.setText(_translate("Loader", "Refresh"))
